...
def play_sudoku_game(size, current_drinks, invited_players, user_name):
    ...
    while no_of_holes > 0:
        ...
        if puzzle_board[i][j] != 0:
            print("빈칸이 아닙니다.")
            continue
        n = get_integer(f"숫자(1~{len(puzzle_board)}): ", 1, len(puzzle_board), end="\n")
...
def sudoku_game(current_drinks, invited_players, user_name):
    ...
    if input("게임 설명이 필요하신가요? (0: 아니오, 1: 예): ").strip() == '1':
        ...
        print("2. 빈 칸의 갯수가 달라집니다. 난이도를 선택하세요 (초급, 중급, 고급).")
        print("3. 빈칸에 올바른 숫자를 입력하여 보드를 완성하세요.")
        print("4. 올바른 숫자를 입력하면 점수를 얻습니다.")
        print("5. 잘못된 숫자를 입력하면 다시 시도해야 합니다.")
        print("\n벌칙 설명:")
        print("1. 초급 난이도에서 2분 이하로 완료하면 🍺나 빼고 아무나 랜덤 한 잔~!🍺")
        print("2. 중급 난이도에서 5분 이하로 완료하면 🍺나 빼고 아무나 랜덤 한 잔~!🍺")
        print("3. 모든 난이도에서 2~5분(초급), 5~10분(중급) 걸리면 🍺다 같이 아무나 랜덤 한 잔~!🍺")
